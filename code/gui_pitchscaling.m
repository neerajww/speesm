...
f.Name = 'Time-varying harmonic demodulation';
...
fpanel_1 = figure('Visible','off','Position',[360,800,400,400]);
...
%         qhd.file{2} = 'MTalker_1_word_2.wav';
    ...
    function fileload_1_Callback(source,eventdata)
        ...
        % read audio
        [qhd.x, qhd.Fs] = audioread(fullfile(qhd.path,qhd.file));
        qhd.x = resample(qhd.x,16e3,qhd.Fs);
        qhd.Fs = 16e3;
        qhd.taxis = (0:length(qhd.x)-1)/qhd.Fs;
        % plot waveform
        plot(ha1,qhd.taxis,qhd.x);
        set(get(ha1, 'xlabel'), 'string', 'time [in s]');
        set(get(ha1, 'ylabel'), 'string', 'amplitude');
        set(get(ha1, 'title'), 'string', 'Original signal');
        set(ha1,FS,FSval,'box','on');
        axes(ha1)
        axis tight
        % plot spectrogram
        [xSTFT] = tSTFT(qhd.x,qhd.Fs,wmsec,wtype,hop_frac,0);
        nframes = size(xSTFT,2);
        nfft = size(xSTFT,1);
        wlen = wmsec*qhd.Fs;
        hop  = fix((wlen-1)/hop_frac);
        ...
        clims = [-db_down 0];
        ylim(ha2,[0 8])
        imagesc(ha2,staxis,faxis,20*log10(abs(xSTFT(fix(faxis*nfft/qhd.Fs/.001)+1,1:length(staxis))))-max_mag,clims);
        set(get(ha2, 'xlabel'), 'string', 'time [in s]');
        set(get(ha2, 'ylabel'), 'string', 'frequency [in kHz]');
        set(get(ha2, 'title'), 'string', 'Spectrogram');
        set(ha2,FS,FSval,'box','on');
        axes(ha2)
        axis xy
        axis tight
        cmap = cbrewer('seq','Blues',100);
        ...
        loadStatus_1.String = ['Loaded audio: ' qhd{1}.file];
        loadStatus_2.String = ['Loaded audio: ' qhd{2}.file];
        loadStatus_1.Enable = 'off';
    ...
    function fileload_2_Callback(source,eventdata)
        ...
        qhd.taxis = (0:length(qhd.x)-1)/qhd.Fs;
        % plot waveform
        plot(ha3,qhd.taxis,qhd.x);
        set(get(ha3, 'xlabel'), 'string', 'time [in s]');
        set(get(ha3, 'ylabel'), 'string', 'amplitude');
        set(get(ha3, 'title'), 'string', 'Original signal');
        set(ha3,FS,FSval,'box','on');
        axes(ha3)
        axis tight
        % plot spectrogram
        [xSTFT] = tSTFT(qhd.x,qhd.Fs,wmsec,wtype,hop_frac,0);
        ...
        clims = [-db_down 0];
        ylim(ha,[0 8])
        imagesc(ha4,staxis,faxis,20*log10(abs(xSTFT(fix(faxis*nfft/qhd.Fs/.001)+1,1:length(staxis))))-max_mag,clims);
        set(get(ha4, 'xlabel'), 'string', 'time [in s]');
        set(get(ha4, 'ylabel'), 'string', 'frequency [in kHz]');
        set(get(ha4, 'title'), 'string', 'Spectrogram');
        set(ha4,FS,FSval,'box','on');
        axes(ha4)
        axis xy
        axis tight
        cmap = cbrewer('seq','Blues',100);
        colormap(cmap);
        % enable follow-up buttons
        loadStatus.String = ['Loaded audio: ' qhd.file];
        loadStatus.Enable = 'on';
        getF0track.Enable = 'on';
        fileload.Enable = 'off';
    end
    ...
    function getF0track_Callback(source,eventdata) 
       for i = 1:2
            ...
            [f0] = exstraightsource(temp,qhd.Fs);
...
%             qhd.f0track{i}(qhd.f0track{i}>350) = 350;
    ...
    function doMorph_Callback(source,eventdata)
        ...
        [qhd.interp_sigs] = gui_tvh_interpolate(mqhd,qhd.ndivs);
        ...
        fpanel_1 = figure('Visible','on','Position',[360,800,700,700]); %[cx,cy,width,height]        
        xloc_1 = 100;
        xloc_2 = 600;
        yloc_1 = 100;
...
%                     'Callback',playMorph_Callback(qhd.interp_sigs{m,n}.sig,mqhd{1}.Fs));
        ...
        textAxis_x   = uibutton(fpanel_1,'Style','pushbutton',...
            'String','AM fraction from B','Position',[80,50,300,20],...
            'Enable','off','Units','normalized');
        textAxis_y   = uibutton(fpanel_1,'Style','pushbutton',...
            'String','FM fraction from B','Position',[50,100,20,300],...
            'Rotation',90,'Enable','off','Units','normalized');
        % ----- enable saveSound
        saveSound.Enable = 'on';
    end    
